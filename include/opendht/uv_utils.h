...
struct Job : std::enable_shared_from_this<Job> {
    ...
    void cancel() {
        if (not uv_is_closing((uv_handle_t*)&timer_)) {
            if (not timer_.data)
                std::cerr << "Job: data not set! " << this << std::endl;
                //timer_.data = new Sp<Job>(shared_from_this());
...
private:
    uv_timer_t timer_;
    uint64_t getTimeout(const duration& d) {
        return std::chrono::duration_cast<std::chrono::milliseconds>(d).count();
    }
    uint64_t getTimeout(const time_point& tp) {
...
struct UdpSocket : public std::enable_shared_from_this<UdpSocket>
{
    ...
    void open(in_port_t port, OnReceive cb) {
        if (not sock.data)
