...
NetworkEngine::NetworkEngine(uv_loop_t* loop, in_port_t port, InfoHash& myid, NetId net, Logger& log, Scheduler& scheduler,
    ...
    network(net), DHT_LOG(log), sock(std::make_shared<UdpSocket>(loop)), scheduler(scheduler)
{
    ...
    sock->open(port, std::bind(&NetworkEngine::processMessage, this, _1, _2, _3));
}
...
NetworkEngine::requestStep(Sp<Request> sreq)
{
    ...
    //node.send(req.msg);
    send(req.msg, (node.reply_time >= now - UDP_REPLY_TIME) ? 0 : MSG_CONFIRM, node.addr);
